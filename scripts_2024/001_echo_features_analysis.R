...
if (
  system_name == "MacBook-Pro-CR-2065.local" |
    stringr::str_detect(string = system_name, "laptop-zim.uni-heidelberg.de")
) {
  lib_path <- .libPaths()[1]
...
# load data ---------------------------------
echo_abfrage_ba_join_nearest_exam <- readRDS(
  file = "./data/abfrage2024/echo_abfrage_ba_join_nearest_exam.rds"
)

lab2023 <- read_excel(
  path = glue(
    "{data_path_bestageing2022}/data/elham2023/2023-11-15-elham_metadat_combined_LAB2023_b.xlsx"
  )
) %>%
  select(
    best_ageing_code,
  ...
  mutate(
    HSTNT = case_when(is.na(HSTNT) ~ HSTNTHP, .default = HSTNT)
  ) %>%
  # drop duplicates
  distinct(best_ageing_code, .keep_all = TRUE) %>%
  # convert to numeric and change "," -> "." for decimals
  mutate(across(!c(best_ageing_code), function(x) {
    as.numeric(gsub(",", ".", x))
  })) %>%
...
# MIRNA DAT
model_data1 <- clean_names(readRDS(
  file = glue('{data_path_BestAgeing}/data_new/model_data1.RDS')
)) # has also multiclass col + diagnoses
...
names(all_mirnas) <- str_replace(
  string = names(all_mirnas),
  pattern = "mi_r",
  replacement = "mir"
)
mirnas_disease <- clean_names(data)
names(mirnas_disease) <- str_replace(
  string = names(mirnas_disease),
  pattern = "mi_r",
  replacement = "mir"
)
rm(data, mirnas)
...
## seq metadat 09-2023
# batch info
hbdx_metadat <- clean_names(readxl::read_excel(
  path = glue(
    '{data_path_bestageing2022}/data/kahraman2023/230907_annotation_chrstoph_reich.xlsx'
  )
)) # has also multiclass col + diagnoses
# detection matrix
det_mat_all_mirnas <- read.table(glue(
  "{data_path_bestageing2022}/data/kahraman2023/det_mat_all_mirnas.txt"
)) %>%
  t() %>%
  as.data.frame() %>%
  clean_names()
colnames(det_mat_all_mirnas) <- str_replace(
  string = colnames(det_mat_all_mirnas),
  pattern = "mi_r",
  replacement = "mir"
)
...
sum_duplicated_samples <- sum(duplicated(hbdx_metadat$customer_id))
index_duplicated_samples <- duplicated(hbdx_metadat$customer_id)
...
checked_duplicated_hbdx <- hbdx_metadat %>%
  ...
  select(customer_id, date_analysis, sample_qc, array_qc) %>%
  arrange(customer_id)
# remove
...
load(glue(
  "{data_path_BestAgeing}/data_research/fromR/researchMiRNAAccession.rda"
))
# check if all mirnas are named the same
researchMiRNAAccession$miRNAName_v21 <- make_clean_names(
  researchMiRNAAccession$miRNAName_v21
) %>%
...
researchMiRNAAccession$miRNAName_v21[
  researchMiRNAAccession$miRNAName_v21 == "hsa_mir_106a_5p"
] <- "hsa_mir_106b_5p"
...
## SURVIVAL DAT
survival_dat <- clean_names(readRDS(glue(
  "{data_path_bestageing2022}/data/202211908_XMELD_abfrage_best_ageing.rds"
))) # %>%
...
control_ids <- read_excel(glue(
  "{data_path_BestAgeing}/data/pheno_controls.xlsx"
)) %>%
...
clean_all_meta_2023_table01 <- readRDS(
  file = glue(
    "{data_path_bestageing2022}/data/disease_identifier_table01/clean_all_meta_2023_table01.rds"
  )
)
...
analysis_dat_ukhd <- all_mirnas %>%
  inner_join(
    clean_all_meta_2023_table01 %>% select(patID, disease, sex, age),
    by = c("pat_id" = "patID")
  ) %>%
...
if (filterDetMatrix == TRUE) {
  ...
  data_long <- filter_dat %>%
    pivot_longer(
      cols = -c(disease, pat_id),
      names_to = "miRNA",
      values_to = "expression"
    )

  # Stratified density plot
  density_plot_filter_eda_stratified <- ggplot(
    data_long,
    aes(x = expression, fill = disease)
  ) +
  ...
  message(glue(
    "After filtering by detection matrix we receieved {sum(!lowExprAll)} miRNA features which we will use for our analysis."
  ))
...
if (runLimmaRemoveBatchEffect == TRUE) {
  ...
  exprs_metadat <- data01 %>%
    ...
    # batch 2
    left_join(
      hbdx_metadat %>% select(customer_id, slide_id, sample_qc, array_qc),
  ...
  if (runPCA_batch == TRUE) {
    ...
    exprs_metadat %>%
      ...
      #coord_fixed(ratio=1)+
      labs(
        title = NULL, # "PCA Plot Colored by Disease",
        x = "PC1",
        y = "PC2"
      ) +
      theme_minimal(base_size = 16, base_family = 'Arial') +
    ...
    if (SAVE.files == TRUE) {
      pca12_beforebatch <- glue(
        "{data_path_bestageing2022}/output/analysis2024/preprocessing/001_pca12_before_batch.svg"
      )
      ggsave(
        filename = pca12_beforebatch,
        plot = plot_PC1_PC2,
        width = 8,
        height = 6,
        units = "in" # default
      )
      pca13_beforebatch <- glue(
        "{data_path_bestageing2022}/output/analysis2024/preprocessing/001_pca13_before_batch.svg"
      )
      ggsave(
        filename = pca13_beforebatch,
        plot = plot_PC1_PC3,
        width = 8,
        height = 6,
        units = "in" # default
  ...
  adjustedMatrix <- removeBatchEffect(
    t(exprs_data),
    batch = batch_slide_id,
    design = modlimma
  ) # collinearity issues with center batch:  one-to-one mapping with any condition
  limma_edata <- adjustedMatrix %>%
    t() %>%
    as.data.frame() %>%
    as_tibble() %>%
  ...
  set.seed(123)
  random_index <- sample(x = 1:nrow(exprs_data), size = 50, replace = FALSE) %>%
    sort() # samples, not miRNAs
  ...
  y_limits <- c(
    min(c(df_original$Expression, df_corrected$Expression)),
  ...
  p2 <- ggplot(df_corrected, aes(x = Sample, y = Expression)) +
    ...
    theme(
      ...
      axis.text.y = element_blank() # remove y labels from 2nd plot
    )

  # Combine plots
  combined_plot <- p1 | p2
  if (SAVE.files == TRUE) {
    boxplot_batch_correct <- glue(
      "{data_path_bestageing2022}/output/analysis2024/preprocessing/001_boxplot_before_after_batch.svg"
    )
    ggsave(
      filename = boxplot_batch_correct,
      plot = combined_plot,
      width = 8,
      height = 6,
      units = "in" # default
  ...
  if (runPCA_batch2 == TRUE) {
    ...
    exprs_metadat <- limma_edata %>%
      bind_cols(pca_df) %>%
      mutate(batch_center = batch_center, disease = data01$disease)
    ...
    if (SAVE.files == TRUE) {
      pca12_afterbatch <- glue(
        "{data_path_bestageing2022}/output/analysis2024/preprocessing/001_pca12_after_batch.svg"
      )
      ggsave(
        filename = pca12_afterbatch,
        plot = plot_PC1_PC2_corrected,
        width = 8,
        height = 6,
        units = "in" # default
      )
      pca13_afterbatch <- glue(
        "{data_path_bestageing2022}/output/analysis2024/preprocessing/001_pca13_after_batch.svg"
      )
      ggsave(
        filename = pca13_afterbatch,
        plot = plot_PC1_PC3_corrected,
        width = 6,
        height = 6,
        units = "in" # default
      )
...
if (SAVE.files == TRUE) {
  path2dataprocessed <- glue(
    "{data_path_bestageing2022}/output/analysis2024/data/001_data01_2024_features.rds"
  )
...
analysis_dat_ukhd2024_merged <- analysis_dat_ukhd2024_merged %>%
  mutate(
    Aortenwurzel = ifelse(
      Aortenwurzel < 20 | Aortenwurzel > 55,
      NA,
      Aortenwurzel
    ),
    Linker_Vorhof = ifelse(
      Linker_Vorhof < 25 | Linker_Vorhof > 60,
      NA,
      Linker_Vorhof
    ),
    max_wall_thickness = ifelse(
      max_wall_thickness < 5 | max_wall_thickness > 30,
      NA,
      max_wall_thickness
    ),
    ...
    SV_Funktion_EE_Ruhe = ifelse(
      SV_Funktion_EE_Ruhe < 3 | SV_Funktion_EE_Ruhe > 35,
      NA,
      SV_Funktion_EE_Ruhe
    ),
    SV_Funktion_MAPSE = ifelse(
      SV_Funktion_MAPSE < 0.5 | SV_Funktion_MAPSE > 3,
      NA,
      SV_Funktion_MAPSE
    )
  )
...
echo_cor_results <- calculate_correlations(
  echo_traits,
  miRNAs,
  analysis_dat_ukhd2024_merged
)
lab_cor_results <- calculate_correlations(
  lab_traits,
  miRNAs,
  analysis_dat_ukhd2024_merged
)
...
saveRDS(
  echo_cor_results,
  file = glue(
    "{data_path_bestageing2022}/output/analysis2024/correlation_univar/echo_cor_results.rds"
  )
)
saveRDS(
  lab_cor_results,
  file = glue(
    "{data_path_bestageing2022}/output/analysis2024/correlation_univar/lab_cor_results.rds"
  )
)
...
echo_plot <- top_echo_features_ordered %>%
  ggplot(aes(
    reorder_within(x = miRNA, by = Correlation, within = New_Trait),
    y = Correlation,
    ymin = Conf_Lower,
    ymax = Conf_Upper
  )) +
...
echo_cor_feature_plot <- glue(
  "{data_path_bestageing2022}/output/analysis2024/correlation_features/001_echo_cor_feature.svg"
)
ggsave(
  filename = echo_cor_feature_plot,
  plot = echo_plot,
  width = 18,
  height = 15,
  units = "in" # default
...
lab_cor_feature_plot <- glue(
  "{data_path_bestageing2022}/output/analysis2024/correlation_features/001_lab_cor_feature.svg"
)
ggsave(
  filename = lab_cor_feature_plot,
  plot = lab_plot,
  width = 18,
  height = 15,
  units = "in" # default
...
top_echo_features_ordered_gt <- top_echo_features_ordered %>%
  ...
  mutate(
    ...
    Adjusted_P_Value = ifelse(
      Adjusted_P_Value < 0.001,
      "<0.001",
      sprintf("%.3f", Adjusted_P_Value)
    ),
...
gt_table %>%
  gtsave(glue(
    "{data_path_bestageing2022}/output/analysis2024/correlation_features/top_echo_features_ordered_gt.docx"
  ))
...
create_volcano_plot <- function(cor_results, title) {
  ...
  cor_results <- cor_results %>%
    mutate(
      Significance_Level = -log10(Adjusted_P_Value),
  ...
  plot <- ggplot(cor_results, aes(x = Correlation, y = Significance_Level)) +
    ...
    scale_color_manual(
      values = c("Significant" = "#dc3912", "Not Significant" = "grey")
    ) +
...
volcano_cor_echo <- glue(
  "{data_path_bestageing2022}/output/analysis2024/correlation_features/001_volcano_cor_echo.svg"
)
ggsave(
  filename = volcano_cor_echo,
  plot = echo_plot,
  width = 5,
  height = 4,
  units = "in" # default
...
volcano_cor_lab <- glue(
  "{data_path_bestageing2022}/output/analysis2024/correlation_features/001_volcano_cor_lab.svg"
)
ggsave(
  filename = volcano_cor_lab,
  plot = lab_plot,
  width = 5,
  height = 4,
  units = "in" # default
...
volcano_facet_wrap_echo <- create_combined_volcano_plot_facet_wrap(
  echo_cor_results,
  ""
)
print(volcano_facet_wrap_echo)

volcano_cor_echo_facet <- glue(
  "{data_path_bestageing2022}/output/analysis2024/correlation_features/001_volcano_facet_cor_echo.svg"
)
ggsave(
  filename = volcano_cor_echo_facet,
  plot = volcano_facet_wrap_echo,
  width = 8,
  height = 8,
  units = "in" # default
)
...
volcano_facet_wrap_lab <- create_combined_volcano_plot_facet_wrap(
  lab_cor_results,
  ""
)
print(volcano_facet_wrap_lab)

volcano_cor_lab_facet <- glue(
  "{data_path_bestageing2022}/output/analysis2024/correlation_features/001_volcano_facet_cor_lab.svg"
)
ggsave(
  filename = volcano_cor_lab_facet,
  plot = volcano_facet_wrap_lab,
  width = 8,
  height = 8,
  units = "in" # default
)
...
heatmap_cor_traits <- glue(
  "{data_path_bestageing2022}/output/analysis2024/correlation_features/001_heatmap_cor_features.svg"
)
ggsave(
  filename = heatmap_cor_traits,
  plot = heatmap_cor_coefficient_traits,
  width = 8,
  height = 8,
  units = "in" # default
)
...
calculate_regressions <- function(traits, miRNAs, data) {
  ...
  for (trait in traits) {
    for (miRNA in miRNAs) {
      ...
      subset_data <- data %>%
        select(all_of(c(trait, miRNA, "sex", "age"))) %>%
        drop_na()

      if (nrow(subset_data) > 2) {
        # Fit linear model: trait ~ miRNA + sex + age
        model <- lm(
          formula = paste(trait, "~", miRNA, "+ sex + age"),
          data = subset_data
        )
        ...
        if (nrow(coef_info) == 1) {
          # Ensure there is exactly one row of data for the miRNA term
          reg_results <- rbind(
            reg_results,
            data.frame(
              Trait = trait,
...
# Calculate
echo_reg_results <- calculate_regressions(
  echo_traits,
  miRNAs,
  analysis_dat_ukhd2024_merged
)
lab_reg_results <- calculate_regressions(
  lab_traits,
  miRNAs,
  analysis_dat_ukhd2024_merged
)
...
saveRDS(
  echo_reg_results,
  file = glue(
    "{data_path_bestageing2022}/output/analysis2024/regression_univar/echo_reg_results.rds"
  )
)
saveRDS(
  lab_reg_results,
  file = glue(
    "{data_path_bestageing2022}/output/analysis2024/regression_univar/lab_reg_results.rds"
  )
)

echo_reg_results <- readRDS(
  file = glue(
    "{data_path_bestageing2022}/output/analysis2024/regression_univar/echo_reg_results.rds"
  )
)
lab_reg_results <- readRDS(
  file = glue(
    "{data_path_bestageing2022}/output/analysis2024/regression_univar/lab_reg_results.rds"
  )
)
...
create_volcano_plot <- function(reg_results, title) {
  ...
  plot <- ggplot(
    reg_results,
    aes(x = Regression_Coefficient, y = -log10(Adjusted_P_Value))
  ) +
...
volcano_regression_echo_facet <- glue(
  "{data_path_bestageing2022}/output/analysis2024/regression_univar/001_volcano_facet_regression_echo.svg"
)
ggsave(
  filename = volcano_regression_echo_facet,
  plot = echo_volcano_plot,
  width = 10,
  height = 6,
  units = "in" # default
)

volcano_regression_lab_facet <- glue(
  "{data_path_bestageing2022}/output/analysis2024/correlation_features/001_volcano_facet_regression_lab.svg"
)
ggsave(
  filename = volcano_regression_lab_facet,
  plot = lab_volcano_plot,
  width = 8,
  height = 8,
  units = "in" # default
)
...
create_dot_plot <- function(reg_results) {
  ...
  reg_results <- reg_results %>%
    group_by(Trait) %>%
    dplyr::mutate(
      Scaled_Coefficient = min_max_scale(abs(Regression_Coefficient))
    ) %>%
    ungroup()
  ...
  ggplot(
    reg_results,
    aes(x = Trait, y = miRNA, size = Scaled_Coefficient, color = Significance)
  ) +
...
dotplot_regression_traits <- glue(
  "{data_path_bestageing2022}/output/analysis2024/correlation_features/001_dotplot_regression_features.svg"
)
ggsave(
  filename = dotplot_regression_traits,
  plot = dot_plot_regression,
  width = 12,
  height = 10,
  units = "in" # default
)
...
top_echo_features_ordered_gt_cor_reg <- top_echo_features_ordered %>%
  ...
  # prepare reg cols
  mutate(
    Regression_Coefficient = sprintf(
      "%.3f [%0.3f; %0.3f]",
      Regression_Coefficient,
      Conf_Lower_Reg,
      Conf_Upper_Reg
    ...
    P_Value_Reg = ifelse(
      P_Value_Reg < 0.001,
      "<0.001",
      sprintf("%.3f", P_Value_Reg)
    ),
    Adjusted_P_Value_Reg = ifelse(
      Adjusted_P_Value_Reg < 0.001,
      "<0.001",
      sprintf("%.3f", Adjusted_P_Value_Reg)
    ),
  ...
  select(
    ...
    -Sample_Size,
    -Significance,
    -c(Conf_Lower_Reg, Conf_Upper_Reg, Significance_Reg)
  ) # Remove now combined columns
...
gt_table %>%
  gtsave(glue(
    "{data_path_bestageing2022}/output/analysis2024/top_echo_features_ordered_gt_cor_reg.docx"
  ))
...
if (run_regression) {
  ...
  for (trait in echo_traits) {
    ...
    if (!is.null(race_results)) {
      ...
      filename <- glue(
        "./output/analysis2024/regression/tuning_results/001_list_tune_race_results_{trait}.rds"
      )
  ...
  if (file.exists(filename)) {
    ...
    if (!is.null(results) && !is.null(results$race_results)) {
      ...
      plot <- autoplot(
        ...
        geom_text(
          aes(y = mean - 0.05, label = wflow_id),
          angle = 45,
          hjust = 1,
          size = 3
        ) +
        labs(
          title = paste("Model Evaluation for", trait),
...
for (trait in echo_traits) {
  ...
  for (model_idx in 1:nrow(results$race_results)) {
    ...
    results_list[[trait]][[wflow_id]] <- process_model(
      results,
      trait,
      model_idx
    )
...
results_list <- readRDS(
  file = glue(
    "{data_path_bestageing2022}/output/analysis2024/regression/evaluation/results_list.rds"
  )
)
...
for (trait in names(results_list)) {
  ...
  r_squared_values <- sapply(results_list[[trait]], function(model) {
    model$R_squared
  })
  r_squared_values_traintest <- sapply(results_list[[trait]], function(model) {
    model$train_test_metrics_grouped$.estimate[7]
  })
...
performance_measures_combined <- lapply(names(results_list), function(trait) {
  lapply(names(results_list[[trait]]), function(model) {
    if (
      "train_test_metrics_grouped" %in% names(results_list[[trait]][[model]])
    ) {
...
performance_measures_combined <- readRDS(
  file = glue(
    "{data_path_bestageing2022}/output/analysis2024/regression/performance_measures/performance_measures.rds"
  )
)
performance_measures_combined
...
performance_measures_combined |>
  filter(type == "traintest", .metric == "rsq", trait %in% c("LV_EDD", "LV_EF"))


performance_measures_combined |>
  filter(type %in% c("test"), .metric == "rsq", model == "normalized_LM") |>
  select(.estimate) |>
  max()
...
for (trait in echo_traits) {
  ...
  for (model_idx in 1:nrow(results$race_results)) {
    ...
    vip_results_list[[trait]][[wflow_id]] <- calculate_feature_importance(
      results,
      trait,
      model_idx,
      n_bootstraps = 100
...
vip_results_list <- readRDS(glue(
  "{data_path_bestageing2022}/output/analysis2024/regression/vip_results/vip_results_list.rds"
))
...
performance_measure_plot <- ggplot(
  traintest_data,
  aes(x = trait, y = .estimate, color = model)
) +
...
ggsave(
  filename = glue(
    "{data_path_bestageing2022}/output/analysis2024/regression/evaluation/performance_measure_plot.svg"
  ),
  plot = performance_measure_plot,
  width = 8,
  height = 10,
  units = "in" # default
)
...
base_path <- glue(
  "{data_path_bestageing2022}/output/analysis2024/regression/evaluation/prediction_error"
)
...
for (trait in names(results_list)) {
  ...
  for (model in names(results_list[[trait]])) {
    ...
    if (!is.null(plot)) {
      ...
      warning(glue(
        "Prediction error plot not found for {trait} in model {model}"
      ))
    }
  }
}
...
base_path_vip <- glue(
  "{data_path_bestageing2022}/output/analysis2024/regression/vip_results"
)
...
table_mirna_top50bm_score_alldiseases <- readRDS(
  file = glue(
    "{data_path_bestageing2022}/data-literature/miRetrieve/20240125top50mirnas_all_diseases_pmids_gpt.rds"
  )
)
...
mirna_disease_mapping <- table_mirna_top50bm_score_alldiseases %>%
  ...
  pivot_wider(
    names_from = Topics,
    values_from = Topics,
    values_fill = list(Topics = FALSE),
    values_fn = list(Topics = function(x) TRUE)
  ) %>%
  select(-row_id)
...
for (trait in names(vip_results_list)) {
  ...
  for (model in names(vip_results_list[[trait]])) {
    ...
    importance_data <- vip_results_list[[trait]][[
      model
    ]]$results_bootstrapped_importance
    ...
    if (!is.null(importance_data)) {
      ...
      warning(glue(
        "Bootstrapped importance results not found for {trait} in model {model}"
      ))
    }
  }
}
...
gt_table %>%
  gtsave(glue(
    "{data_path_bestageing2022}/output/analysis2024/regression/vip_results/vip_results.docx"
  ))
