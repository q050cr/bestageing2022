...
# update in 003d script: compared to 003c this script considers only a subgroup analysis that removes batch center (only possible if both disease and controls were sampled from same center)
...
for (i in 1:nrow(all_combis)) {
  ...
  # load disease specific data from 001d ----------------------------------------------------------
  # specific to this script (preprocessing, filtered miRNAs with Detection Matrix, batch effects removed)
  path2dataprocessed <- glue("{data_path_bestageing2022}/data/Rdata/processed_disease_data/001d_{all_combis$diseases[i]}_data01.rds")
  ...
  ## SAVE RACE RESULTS -----------------------------------------
  if(SAVE.files == TRUE) {
    filename_tune_race_results <- glue("{data_path_bestageing2022}/output/tuning_results/{all_combis$diseases[i]}/003d_{Sys.Date()}_tune_race_results_repeats_{no_repeats}_folds_{no_folds}_{text_disease}_analysis_{str_to_upper(all_combis$analysis[i])}_miRetrieve_{miRetrieveBiomarker}_randomMIR_{random_selection}.rds")
