...
for(disease in disease_vector) {
  de.results <- readRDS(dplyr::last(list.files(path = "../output/de_results", pattern = paste0("_de_results_DISEASE_", disease), full.names = TRUE)))
  ...
  log2folds_thresholds <- c(0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1)
  for (i in log2folds_thresholds) {
    if ( sum(de.results$diffexpressed != "NO"  ) < 15 ) {
    de.results$diffexpressed[de.results$log2FoldChange > i & 
                               de.results$padj < 0.05] <- "UP"
    # if log2Foldchange < -0.6 and pvalue < 0.05, set as "DOWN"
    de.results$diffexpressed[de.results$log2FoldChange < - i & 
                               de.results$padj < 0.05] <- "DOWN"
  ...
  # ONLY label miRNAs that are also known from LITERATURE (maybe also from MIGWAS later)
  ...
  ggplot(data=de.results, aes(x=log2FoldChange, y=-log10(padj), col=diffexpressed, label=delabel)) + 
        ...
        labs(title = paste0("Volcano plot of DE miRNAs in ", disease),
  ...
  print(volcano_1)
  filename.volcano <-paste0("../output/plots/de_analysis/", Sys.Date(), "_de_volcano_DISEASE_", disease, ".png")
