...
#  Load libraries
require(miRetrieve)
packageVersion('miRetrieve')
require(glue)
require(magrittr) # Load magrittr for %>%
require(ggplot2) # Load ggplot2 for plotting
require(svglite)
require(easyPubMed) # Load easyPubMed, access to PubMed
require(rcrossref)
require(dplyr) # Data wrangling
require(tidyr) # Data wrangling
require(patchwork) # Group graphs
...
query_list_trials[["ACS"]] <- glue::glue(
  {
    query_list[["ACS"]]
  },
  {
...
query_list_trials[["CAD"]] <- glue::glue(
  {
    query_list[["CAD"]]
  },
  {
...
query_list_trials[["DCM"]] <- glue::glue(
  {
    query_list[["DCM"]]
  },
  {
...
query_list_trials[["HFrEF"]] <- glue::glue(
  {
    query_list[["HFrEF"]]
  },
  {
...
for (disease in seq_along(diseases)) {
  ...
  # Number of abstracts
  print(glue::glue(
    "{length(unique(df[['PMID']]))} unique abstracts with extracted miRNA names mentioned at least twice/ abstract for disease: {diseases[disease]}"
  ))
  # Number of rows
  nrow(df)
  ...
  # Extract all miRNAs
  df_comparison <-
    read_pubmed(file, topic = diseases[disease]) %>%
  ...
  df_comparison_manual_pubmed <- df_comparison %>%
    ...
    mutate(
      unrelated_topic01 = NA,
      direction_upreg_downreg = NA,
      english = "yes",
      related_topic = NA,
      direction_upreg_downreg = NA,
      primary_literature = NA,
  ...
  df_count_mixed <- bind_rows(df_count_letter, df_count_no_letter) %>% arrange(desc(miRetrieve)) %>% slice(1:30) %>% mutate(miRNA = paste0("hsa-", miRNA))
  count_plot_mixed <- count_plot_fn(df_count = df_count_mixed)
  print(count_plot_mixed)

  if (saveFILE == TRUE) {
    saveRDS(
      object = df_comparison, # all miRNAs in df
    ...
    saveRDS(
      object = df_count_both, # count top 30 mirnas with letter suffix and top 30 without
    ...
    ggsave(
      filename = paste0(filename.fig1, ".svg"),
      plot = combined,
      width = 10,
      height = 6,
      units = "in" # default
    )
    filename.fig1_mixed <- glue::glue(
      "{data_path_bestageing2022}/output/plots/miRetrieve/{diseases[disease]}/{Sys.Date()}{subset}-figure1-compareTop30_mixed_letter"
    )
    ggsave(
      filename = paste0(filename.fig1_mixed, ".svg"),
      plot = count_plot_mixed,
      width = 8,
      height = 6,
      units = "in" # default
    )
  ...
  disease_biomarker <- calculate_score_biomarker(
    df_comparison,
  ...
  if (saveFILE == TRUE) {
    ...
    saveRDS(
      object = disease_biomarker_with_article_count, # new for 03miRetrieve_topmirnas_all_diseases.R
      file = glue::glue(
        "{data_path_bestageing2022}/data-literature/miRetrieve/{tolower(diseases[disease])}/{Sys.Date()}{subset}-disease_biomarker_with_article_count.rds"
      )
    )

    filename.fig7 <- glue::glue(
      "{data_path_bestageing2022}/output/plots/miRetrieve/{diseases[disease]}/{Sys.Date()}{subset}-figure7-mostfrequent_potential_miRNA_biomarker"
    )
    ggsave(
      filename = paste0(filename.fig7, ".svg"),
      plot = biomarker_plot,
      width = 6,
      height = 6,
      units = "in" # default
...
# improve overview
disease_stop <- generate_stopwords(
  c(
    "atherosclerosis",
    "atherosclerotic",
    "heart failure",
...
mir133_1 <- plot_mir_terms(df_acs, "miR-133", stopwords = disease_stop, top = 15, title = "Top single terms for miR-133 in ACS")

# Plot 2-grams for miR-133
mir133_2 <- plot_mir_terms(df_acs, "miR-133", token = "ngrams", n = 2, stopwords = disease_stop, top = 14, title = "Top 2-grams for miR-133 in ACS")

# Plot single word terms for miR-21
mir21_1 <- plot_mir_terms(df_acs, "miR-21", stopwords = disease_stop, top = 11, title = "Top single terms for miR-21 in ACS")

# Plot 2-grams for miR-21
mir21_2 <- plot_mir_terms(df_acs, "miR-21", token = "ngrams", n = 2, stopwords = disease_stop, top = 13, title = "Top 2-grams for miR-21 in acs")
...
plot_target_mir_scatter(df_targets_acs_mir, mir = c("miR-133", "miR-21"), alpha = 1, title = "Targets of miR-21 and miR-133 in ACS")


# Disease Associations ----------------------------------------------------

# Top terms in DCM
plot_mir_count(df_dcm, title = "Most frequently mentioned miRNAs in DCM")


# Figure 6: Compare miR associations between Diseases ---------------------

# Generate stop words for miR-21 comparison to improve overview
comparison_stop <- generate_stopwords(
  c("dependent manner", "vascular endothelial", "peripheral blood", "serum mir", "serum levels", "tensin homolog", "phosphatase tensin", "α"),
  combine_with = stopwords_miretrieve
)
...
comp_mir21_1 <- compare_mir_terms(df_acs_dcm, "miR-21", top = 7, title = "Comparison of miR-21 single term association", stopwords = comparison_stop) +
  ggplot2::scale_fill_manual(values = c("#F5793A", "#85C0F9"), name = "Topic")

# Compare shared terms, 2-gram
comp_mir21_2 <- compare_mir_terms(
  df_acs_dcm,
  "miR-21",
  token = "ngrams",
  n = 2,
