...
top2models <- rankings$model[1:2]
for (i in 1:length(all_workflows$info)) {
  if (all_workflows$info[[i]]$model == top2models[1]) {
    topmodel1 <- all_workflows$wflow_id[i]
  }
  
    if (all_workflows$info[[i]]$model == top2models[2]) {
    topmodel2 <- all_workflows$wflow_id[i]
  }
}
```
...
autoplot(grid_results, id = topmodel1, metric = "roc_auc")+
  ylab("ROC-AUC")+
...
autoplot(grid_results, id = topmodel2, metric = "roc_auc")+
  ylab("ROC-AUC")+
  labs(title="Hyperparameter Performance - Racing Approach",
       subtitle = topmodel2)+
  ggthemes::theme_few()
...
test_results1 %>%
  ...
  labs(title = paste0("ROC Curve (Predict ", text_disease, ")"),
       subtitle = paste0(topmodel1, " - Test set") ) +
  annotate("text", x = 0.4, y=0.2, label = paste(topmodel1, "-AUC=", round(auc_test,3), sep = ""), size=3, hjust = 0) +
  annotate("text", x = 0.4, y=0.15, label = paste(topmodel1, "-", ci.auc.text, sep = ""), size=3, hjust = 0) +
  ggthemes::theme_few()+
...
gbm::calibrate.plot(y = predictions$disease, p = predictions[[pred_string]], 
                    main= paste0(topmodel1, " Calibration Plot"), 
...
for (i in seq_along(1:nrow(race_results)) ) {
  ...
  # CARET CONF MATRIX
  caret_conf_mat[[i]] <-  caret::confusionMatrix(predictions_loop[[i]]$.pred_class_youden, predictions_loop[[i]]$disease, positive = params$disease)
