...
# detection matrix
hbdx_detmat <- clean_names(readxl::read_excel(path = glue('{data_path_bestageing2022}/data/kahraman2023/230907_annotation_chrstoph_reich.xlsx')))  # has also multiclass col + diagnoses
test <- read.table("/Volumes/T7CR/data/bestageing2022/data/kahraman2023/det_mat_all_mirnas.txt")
...
for (i in 1:nrow(all_combis[all_combis[["analysis"]] == "full", ])) {  # only consider all miRNAs for DE (not research miRNAs only)
  ...
  if (filterBackground == TRUE){
    filter_dat <- data01 %>% select(-c(pat_id, age,sex))
    
    # EDA - Plotting a density of average log2 expressions 
    data_long <- filter_dat %>% 
      pivot_longer(cols = -disease, names_to = "miRNA", values_to = "expression")
    ...
    if (SAVE.files ==TRUE) {
      density_ridges_plot_filter_eda_stratified_path <- glue("{data_path_bestageing2022}/output/plots/preprocessing/filtering_miRNA/{disease}_density_ridges.svg")
    ...
    lowExprControl <- colMeans(filter_dat[filter_dat$disease == "control", -1]) <= threshold
    
    # Filter for disease samples
    lowExprDisease <- colMeans(filter_dat[filter_dat$disease == disease, -1]) <= threshold
    ...
    filteredData <- filter_dat[, c(TRUE, !toFilter)]
    
    data01 <- data01 %>% 
      select(1:4) %>% 
      dplyr::bind_cols(filteredData[, -1])
  }
  ...
  if(runLimmaRemoveBatchEffect == TRUE) {
    ...
    if(runPCA_batch == TRUE) {
      ...
      if (SAVE.files ==TRUE) {
        pca12_beforebatch <- glue("{data_path_bestageing2022}/output/plots/preprocessing/pca_batch_corr/{disease}_pca12_before_batch.svg")
        ggsave(filename = pca12_beforebatch, plot = plot_PC1_PC2, 
               width = 8, height = 6, 
               units = "in"  # default
        )
        pca13_beforebatch <- glue("{data_path_bestageing2022}/output/plots/preprocessing/pca_batch_corr/{disease}_pca13_before_batch.svg")
    ...
    p1 <- ggplot(df_original, aes(x = Sample, y = Expression)) +
      geom_boxplot(aes(group = Sample), alpha=0.7) +
      theme_minimal(base_size = 16, base_family = 'Arial')+
    ...
    combined_plot <- p1 | p2
    if (SAVE.files ==TRUE) {
      boxplot_batch_correct <- glue("{data_path_bestageing2022}/output/plots/preprocessing/boxplot_batch_corr/{disease}_boxplot_before_after_batch.svg")
    ...
    if(runPCA_batch2 == TRUE) {
      ...
      if (SAVE.files ==TRUE) {
        pca12_afterbatch <- glue("{data_path_bestageing2022}/output/plots/preprocessing/pca_batch_corr/{disease}_pca12_after_batch.svg")
        ggsave(filename = pca12_afterbatch, plot = plot_PC1_PC2_corrected, 
               width = 8, height = 6, 
               units = "in"  # default
        )
        pca13_afterbatch <- glue("{data_path_bestageing2022}/output/plots/preprocessing/pca_batch_corr/{disease}_pca13_after_batch.svg")
  ...
  if (runTests == TRUE){  #takes time
    ...
    if (SAVE.files ==TRUE) {
      filename.de.tibble <- glue("{data_path_bestageing2022}/output/de_results/{disease}/de_results_batch_corrected.rds")
      saveRDS(object = de.results, file = filename.de.tibble)
      filename.logmedians <- glue("{data_path_bestageing2022}/output/de_results/{disease}/results_logmedians_batch_corrected.rds")
    ...
    if (SAVE.files ==TRUE) {
      qqplot_pvalues_path <- glue("{data_path_bestageing2022}/output/plots/qqplot/de/{disease}_de_qqplot.svg")
      ggsave(filename = qqplot_pvalues_path, plot = qqplot_pvalues, 
             width = 6, height = 6, 
             units = "in"  # default
      )
    }
    ...
    if (n.sv != 0 & exists(x = "n.sv")) {
      ...
      if (SAVE.files ==TRUE) {
        qqplot_pvalues_path_sva <- glue("{data_path_bestageing2022}/output/plots/qqplot/de/{disease}_de_qqplot_sva_included.svg")
