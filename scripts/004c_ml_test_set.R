...
if(RUN.test == TRUE) {
  for (i in 1:nrow(all_combis)){
    ...
    for (models in seq_along(1:nrow(race_results)) ) {
      ...
      if(SAVE.files == TRUE) {
        filename_plot_ROC_wflow_id <- 
          glue("{data_path_bestageing2022}/output/plots/roc_curves/{all_combis$diseases[i]}/004c_ROC_{wflow_id}_analysis_selected_analysis.svg")
    ...
    filename_df_performance_summary_sens09 <- 
      glue("{data_path_bestageing2022}/output/performance_summary_df/{all_combis$diseases[i]}/004c_performance_summary_table_analysis_selected_analysis.rds")
...
for (i in 1:nrow(all_combis)){
  ...
  for (models in seq_along(1:nrow(race_results)) ) {
    ...
    saveRDS(vip1, file = glue("{data_path_bestageing2022}/output/feature_importance/{all_combis$diseases[i]}/004c_selected_{wflow_id}_vip1.rds"))
    ...
    saveRDS(features_importance_table, file = glue("{data_path_bestageing2022}/output/feature_importance/{all_combis$diseases[i]}/004c_selected_{wflow_id}_features_importance_table.rds"))
    ...
    vip_plot_filename <- glue("{data_path_bestageing2022}/output/plots/feature_importance/{all_combis$diseases[i]}/004c_{wflow_id}_variable_imp_plot_selectedmiRetrieve")
    ggsave(
  ...
  df_intersections <- df_intersections %>% 
    ...
    arrange(desc(Count))
  df_intersections
  saveRDS(object = df_intersections, file = glue("{data_path_bestageing2022}/output/feature_importance/{all_combis$diseases[i]}/004c_selected_analysis_m50_vars_intersection.rds"))
  
  # docx
  file_path <- glue("{data_path_bestageing2022}/output/feature_importance/{all_combis$diseases[i]}/004c_selected_analysis_m50_selected_vars_intersection.docx")
