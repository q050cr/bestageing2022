...
for (i in 1:nrow(all_combis[all_combis[["analysis"]] == "full", ])) {  # only consider all miRNAs for DE (not research miRNAs only)
  ...
  if (filterDetMatrix == TRUE){
    ...
    if (SAVE.files ==TRUE) {
      density_ridges_plot_filter_eda_stratified_path <- glue("{data_path_bestageing2022}/output/plots/preprocessing/filtering_miRNA/001c_{disease}_density_ridges.svg")
  ...
  if(runLimmaRemoveBatchEffect == TRUE) {
    ...
    if(runPCA_batch == TRUE) {
      ...
      if (SAVE.files ==TRUE) {
        pca12_beforebatch <- glue("{data_path_bestageing2022}/output/plots/preprocessing/pca_batch_corr/001c_{disease}_pca12_before_batch.svg")
        ggsave(filename = pca12_beforebatch, plot = plot_PC1_PC2, 
               width = 8, height = 6, 
               units = "in"  # default
        )
        pca13_beforebatch <- glue("{data_path_bestageing2022}/output/plots/preprocessing/pca_batch_corr/001c_{disease}_pca13_before_batch.svg")
    ...
    combined_plot <- p1 | p2
    if (SAVE.files ==TRUE) {
      boxplot_batch_correct <- glue("{data_path_bestageing2022}/output/plots/preprocessing/boxplot_batch_corr/001c_{disease}_boxplot_before_after_batch.svg")
    ...
    if(runPCA_batch2 == TRUE) {
      ...
      if (SAVE.files ==TRUE) {
        pca12_afterbatch <- glue("{data_path_bestageing2022}/output/plots/preprocessing/pca_batch_corr/001c_{disease}_pca12_after_batch.svg")
        ggsave(filename = pca12_afterbatch, plot = plot_PC1_PC2_corrected, 
               width = 8, height = 6, 
               units = "in"  # default
        )
        pca13_afterbatch <- glue("{data_path_bestageing2022}/output/plots/preprocessing/pca_batch_corr/001c_{disease}_pca13_after_batch.svg")
  ...
  if (runTests == TRUE){  #takes time
    ...
    if (SAVE.files ==TRUE) {
      filename.de.tibble <- glue("{data_path_bestageing2022}/output/de_results/{disease}/001c_de_results_batch_corrected.rds")
      saveRDS(object = de.results, file = filename.de.tibble)
      filename.logmedians <- glue("{data_path_bestageing2022}/output/de_results/{disease}/001c_results_logmedians_batch_corrected.rds")
    ...
    if (SAVE.files ==TRUE) {
      qqplot_pvalues_path <- glue("{data_path_bestageing2022}/output/plots/qqplot/de/001c_{disease}_de_qqplot.svg")
    ...
    if (n.sv != 0 & exists(x = "n.sv")) {
      ...
      if (SAVE.files ==TRUE) {
        qqplot_pvalues_path_sva <- glue("{data_path_bestageing2022}/output/plots/qqplot/de/001c_{disease}_de_qqplot_sva_included.svg")
...
for(mydisease in 1:nrow(all_combis[all_combis[["analysis"]] == "full", ])){
  ...
  de_results <- readRDS(file = glue("{data_path_bestageing2022}/output/de_results/{all_combis$diseases[mydisease]}/001c_de_results_batch_corrected.rds"))
  ...
  p1.2 <- ggplot() +
    geom_histogram(data = results_logmedians, aes(x = padj.glm_pca), fill = "white", color = "black", # bins = 500,
    ...
    labs(x = glue("GLM P-value\n({toupper(adjusting_method)} adjusted)"), y = "Frequency") +
    #theme_classic()
    theme_minimal(base_size = 16, base_family = 'Arial')+
    scale_fill_manual(values = thematic::okabe_ito(6)) +
    my_base_theme()
  ...
  if (SAVE.files ==TRUE) {
    fig2_disease_vogel2013_path <- glue("{data_path_bestageing2022}/output/plots/fig02vogel2013/001c_{Sys.Date()}_{toupper(all_combis$diseases[mydisease])}_topdysregulatedmiRetrievemiR.svg")
...
print(fig2vogel2013)
if (SAVE.files ==TRUE) {
  fig2vogel2013_path <- glue("{data_path_bestageing2022}/output/plots/fig02vogel2013/001c_{Sys.Date()}_arranged_topdysregulatedmiRetrievemiR.svg")
...
for(mydisease in 1:nrow(all_combis[all_combis[["analysis"]] == "full", ])) {
  ...
  if (SAVE.files ==TRUE ){
    filename.volcano2 <-glue("{data_path_bestageing2022}/output/plots/de_analysis/{all_combis$diseases[mydisease]}/001c_de_volcano_LITERATmiRNAs_ANNOT.svg")
...
venn_plot_poster <- ggvenn(
  list_vann, 
  text_size = 6,
  show_percentage = FALSE, show_elements = FALSE) +
  theme_minimal(base_size = 16, base_family = 'Arial')+
  scale_fill_manual(values = thematic::okabe_ito(6)) +
  scale_color_manual(values = thematic::okabe_ito(6)) +
  theme(
...
venn_plot_poster <- venn_plot_poster + coord_fixed(ratio = 1.3) 
venn_plot_poster
...
if (SAVE.files ==TRUE ){
  ...
  saveRDS(object = venn_plot_high, file = glue("{data_path_bestageing2022}/output/plots/venn_dia/001c_venn_de_mirnas_shared.rds"))
...
if (length(shared_elements_all_diseases) > 0) {
  cat("Shared elements between the sets are:\n")
  cat(paste(shared_elements_all_diseases, collapse = ", "), "\n")
} else {
...
write.csv2(x = intersections_diseases, file = glue("{data_path_bestageing2022}/output/tables/venn_dia/001c_ALL_above_bm_perdisease_venn.csv"))
...
if (SAVE.files ==TRUE ){
  filename.venn_de_mirnas_shared <-glue("{data_path_bestageing2022}/output/plots/venn_dia/001c_venn_de_mirnas_shared.svg")
...
# we experienced p-val inflation and did a lot of analysis to detect any further batch effects
...
outlier_PCA_plot <- ggplot(exprs_metadat, aes(PC1, PC2)) +
  geom_point(aes(color = disease), alpha=0.2, shape=16) +
  geom_point(data = exprs_metadat[outliers, ], aes(PC1, PC2, color=disease), size = 3) +
  labs(title = NULL,  # "PCA Plot Colored by Disease",
       x = "PC1",
       y = "PC2") +
  theme_minimal(base_size = 16, base_family = 'Arial')+
